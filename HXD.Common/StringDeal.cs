...
namespace HXD.Common
{
    ...
    public class StringDeal
    {
        ...
        public static string RemoveUnsafeHtml(object obj)
        {
            ...
            content = Regex.Replace(content, @"(/(\%27)|(\’)|(\-\-)|(\%23)|(#)/ix)", "$1$2", RegexOptions.IgnoreCase);
            return content;
        }
        ...
        public static string GetChecked(object val1, object val2)
        {
            ...
            else if (StrFormat(val1).IndexOf(StrFormat(val2)) >= 0)
            { return " checked=\"checked\""; }
            else
            {
                return "";
        ...
        /// <param name="len"></param>
        /// <returns></returns>
        public static string Substrings1(string str, int len)
        {
            if (str != null && str != "")
            {
                string strings;
