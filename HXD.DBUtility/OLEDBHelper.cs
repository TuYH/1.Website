...
namespace DataLayer
{
    ...
    {
        //数据库连接字符串
        private static string ConnectionString = GetConnectinString();
        ...
        private static string GetConnectinString()
        {
            string AccessString = ConfigurationManager.ConnectionStrings["OledbConnString"].ConnectionString;
            foreach (string x in AccessString.Split(';'))
            {
                if (x.IndexOf("Data Source=") > -1)
                {
                    AccessString = AccessString.Replace(x, "Data Source=" + HttpContext.Current.Server.MapPath(x.Replace("Data Source=", "")));
                }
            }
            return AccessString;
        }
