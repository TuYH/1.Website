<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Model_Set.aspx.cs" Inherits="adminmanage_Menu_Model_Set" %>
...
function ClickChecked(Field)
{
	if($("IsOpen_"+Field).checked==false)
	{
		$("IsList_"+Field).disabled="true";
		$("ListWidth_"+Field).disabled="true";
		$("Title_"+Field).disabled="true";
	}
	else
	{
		$("IsList_"+Field).disabled="";
		$("ListWidth_"+Field).disabled="";
		$("Title_"+Field).disabled="";
	}
}
</script>
</head>
...
<div id="edit">
  ...
  <table border="0" align="center" cellpadding="0" cellspacing="0" class="red_xx table_all">
    <tr>
      <td class="title">信息模型设置</td>
      <td align="right"  nowrap><span class="hui operation">管理导航 - 信息模型设置 | <a href="Menu_Manage.aspx">栏目列表</a></span></td>
    </tr>
  </table>
  ...
  <table align="center" cellspacing="1" class="table table_all" id="table1">
    <tr class="td_bg">
      <td colspan="6" class="redbold">信息模型设置面版</td>
    </tr>
    ...
    <asp:Repeater ID="DBList" runat="server">
      <itemtemplate>
        <tr onmouseover="over()" onclick="change()" onmouseout="out()" class="td_bg">
          ...
          <td width="180"><%#HXD.ModelField.Common.FieldType.GetFieldType(Eval("Type"))%></td>
          <td width="80" align="center"><input name="IsOpen" type="checkbox" id="IsOpen_<%#Eval("Field")%>" value="<%#Eval("Field")%>"<%#GetFieldState(Eval("Field")," checked='checked'")%> onclick="ClickChecked('<%#Eval("Field")%>');" /></td>
          <td width="80" align="center"><input name="IsList" type="checkbox" id="IsList_<%#Eval("Field")%>" value="<%#Eval("Field")%>"<%#GetListState(Eval("Field")," checked='checked'")%> /></td>
          <td width="84" align="center"><input name="ListWidth" type="text" class="input validate-integer" id="ListWidth_<%#Eval("Field")%>" size="3" maxlength="2" value="<%#GetListState(Eval("Field"),"")%>" />
            %</td>
          <td width="220" align="center"><input name="Title" type="text" class="input1" id="Title_<%#Eval("Field")%>" size="50" maxlength="50" value="<%#GetFieldState(Eval("Field"),"")%>" /></td>
        </tr>
		<script language="javascript" type="text/javascript">ClickChecked("<%#Eval("Field")%>");</script>
      </itemtemplate>
    </asp:Repeater>
  </table>
